...
/*    printf("To_see: %d\n", to_see*/
    ...
    if(cur_comb!=NULL){
      ...
      if(abs(cur_var)<n_vars){
        cur_sol = MAXSAT(n_clauses, n_vars, clause_matrix, -abs(cur_var)-1, cur_comb, cur_sol, &cur_maxsat, &n_solutions, first_sat_clauses);
      }
    }

  /*  printf("\nMaster found %d solutions: %d\n", n_solutions, cur_maxsat);*/
...
/*          printf("\nSlave %d found %d solutions: %d \n", i, temp_n_sols, cur_maxsat);*/
        }else{
          if(temp_maxsat>cur_maxsat){
...
int* distributeNodes(int depth, int cur_var, int* prev_comb, int n_vars, int* idle, int* to_see, int* master_var){
    ...
    if(abs(cur_var)<depth && abs(cur_var)<n_vars){
      ...
      if(*to_see < ((*idle)+1)){ /*if there are more processors to "hang" than nodes on this level, "hang" two processors in each of this node's children*/
        ...
        (*idle)=(*idle)-1;
        if(*idle>0){
          sendProblemParams(cur_comb3, -abs(cur_var)-1, n_vars, *idle);
          (*idle)=(*idle)-1;
        }
        else{
          *master_var = -abs(cur_var)-1;
          aux = cur_comb3;
        }
      }
...
void sendProblemParams(int* cur_comb, int cur_var, int n_vars, int proc){
/*  printf("\nNode %d got cur_comb: ", proc); print_sol(cur_comb, n_vars);*/
